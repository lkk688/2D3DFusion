...
class Box3D(object):
    ...
    def in_camera_coordinate(self, is_homogenous=False):
        # 3d bounding box dimensions in camera coorinate
        l = self.l
        w = self.w
        h = self.h

        # 3D bounding box vertices [3, 8]， 8 vertices of a 3D box in the camera coordinates
        ...
        # Rotation
        R = roty(self.ry)  # [3, 3]
        points_3d = R @ box_coord #matrix multiply: https://numpy.org/doc/stable/reference/generated/numpy.matmul.html
...
# Projections
# =========================================================
def project_velo_to_cam2(calib): #Projection from lidar to camera 2
...
def project_cam2_to_velo(calib): #Projection from camera to lidar coordinate
    R_ref2rect = np.eye(4)
    R0_rect = calib['R0_rect'].reshape(3, 3)  # ref_cam2rect
    R_ref2rect[:3, :3] = R0_rect
