...
class WaymoLaserScan:
    ...
    def do_range_projection(self):
        ...
        # field of view down in rad -25->-0.436
        fov_down = self.proj_fov_down / 180.0 * np.pi
        # get field of view total in rad 0.488
...
class WaymoSemLaserScan(WaymoLaserScan):
    ...
    def reset(self):
        """ Reset scan members. """
        super(WaymoSemLaserScan, self).reset()
    ...
    def set_labelfromdict(self, datadict):
        point_labels_all = datadict['point_labels_all']
        # Inner dimensions are [instance_id, semantic_class].
        instance_id_image = point_labels_all[..., 0]
        semantic_class_image = point_labels_all[..., 1]
        self.sem_label = semantic_class_image #shape [totalpoints,]
        self.inst_label = instance_id_image
...
if __name__ == "__main__":
    datafolder = "D:\Dataset\WaymoSemantic"
    sequence = "10017090168044687777_6380_000_6400_000"
    scan_paths = os.path.join(datafolder, sequence, "velodyne")
    # populate the pointclouds, get all Lidar bin files in scan_paths
    scan_names = [os.path.join(dp, f) for dp, dn, fn in os.walk(
        os.path.expanduser(scan_paths)) for f in fn]
    scan_names.sort() #list of Lidar bin files

    seqdatafolder = Path(os.path.join(datafolder, sequence))
    ...
    #scan = WaymoSemLaserScan(project=True)#WaymoLaserScan(project=True)  # project all opened scans to spheric proj
    CFG = yaml.safe_load(open("./KittiSemantic/semantic-kitti.yaml", 'r'))
    ...
    scan.set_labelfromdict(datadict)
