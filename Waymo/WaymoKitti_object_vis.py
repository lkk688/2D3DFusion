...
referenced from kitti_object_vis
"""
...
class Waymokitti_object(object):
    """Load and parse object data into a usable format."""

    def __init__(self, root_dir, folder="training", args=None):
        """root_dir contains training and testing folders"""
        self.root_dir = root_dir
        self.folder = folder
        print(root_dir, folder)
        self.folder_dir = os.path.join(root_dir, folder)
        self.camera_count = 5 # waymo has 5 cameras
...
#             pred_dir = args.preddir
        self.image_dir=[]
        self.imagefiles=[]
        self.image_dir = [os.path.join(self.folder_dir, "image_"+str(i)) for i in range(self.camera_count)] 
        for index in range(self.camera_count):
            self.imagefiles = glob.glob(self.image_dir[index]+"\*.png")
            print("Camera:"+str(index)+" image files:", len(self.imagefiles))
        
        self.label_dir=[]
        self.label_dir=[os.path.join(self.folder_dir, "label_"+str(i)) for i in range(self.camera_count)] 
        for index in range(self.camera_count):
            self.labelfiles = glob.glob(self.label_dir[index]+"\*.txt")
            print("Camera:"+str(index)+" label files:", len(self.labelfiles))
        
        self.calib_dir = os.path.join(self.folder_dir, "calib")
        self.calibfiles = glob.glob(self.calib_dir+"\*.txt")
        print("Calibration files:", len(self.calibfiles))

        #self.depthpc_dir = os.path.join(self.split_dir, "depth_pc")
        self.lidar_dir = os.path.join(self.folder_dir, lidar_dir)
        self.lidarfiles = glob.glob(self.lidar_dir+"\*.bin")
        print("Lidar files:", len(self.lidarfiles))
        
        self.num_samples = len(self.lidarfiles)
