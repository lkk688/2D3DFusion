...
#same to learning_map in semantic-kitti.yaml
...
colors = np.concatenate(
    [colors.astype(np.int), np.ones([colors.shape[0], 1]) * 255], axis=1)
file_id = '000100'
...
if __name__ == '__main__':
    ...
    plot = mlab.points3d(scan[:, 0], scan[:, 1],
                         scan[:, 2], label - 1, mode="point", figure=fig)

    # magic to modify lookup table
    plot.module_manager.scalar_lut_manager.lut._vtk_obj.SetTableRange(
        0, colors.shape[0])
    ...
    mlab.savefig(filename='data/semantic_kitti_label_to_cloud.png')
    mlab.show()
